...
frm_image.columnconfigure(height=1, weight=1)
frm_image.rowconfigure(height=0, weight=1)
frm_image.grid(row=0, columnspan=2, column=1, pady=(10,20), sticky=NSEW)

# Create the frame to hold the image explanation
frm_explanation = ttk.Frame(root)
frm_explanation.columnconfigure(height=1, weight=2)
frm_explanation.rowconfigure(height=0,weight=1)
...
def handle_download_image():
    ...
    apod_id = apod_desktop.add_apod_to_cache() 
    if apod_id != 0:
